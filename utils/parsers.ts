...
export const parsePillars = (json: unknown): SEOPillars => {
    ...
    return {
        ...
        // Content areas (generated by strategy step)
        contentAreas: Array.isArray(safeJson.contentAreas) ? (safeJson.contentAreas as unknown[]).map(safeString).filter(Boolean) : undefined,
        contentAreaTypes: Array.isArray(safeJson.contentAreaTypes)
            ? (safeJson.contentAreaTypes as unknown[]).filter((v): v is 'revenue' | 'authority' => v === 'revenue' || v === 'authority')
            : undefined,
    };
};
...
export const parseTopicalMap = (data: any): TopicalMap => {
    ...
    return {
        ...
        map_name: data.map_name ? safeString(data.map_name) : undefined,
        domain: data.domain ? safeString(data.domain) : undefined,
        created_at: safeString(data.created_at),
        ...
        // Styleguide data (persisted JSONB column)
        styleguide_data: data.styleguide_data && typeof data.styleguide_data === 'object' ? data.styleguide_data : undefined,
