...
const callApi = async <T>(
    ...
    dispatch({ type: 'LOG_EVENT', payload: { service: 'Perplexity', message: `Sending request to ${businessInfo.aiModel}...`, status: 'info', timestamp: Date.now() } });
    ...
    try {
        const response = await fetch(API_URL, {
            ...
            body: JSON.stringify({
                model: businessInfo.aiModel || 'sonar-pro',
                messages: [
